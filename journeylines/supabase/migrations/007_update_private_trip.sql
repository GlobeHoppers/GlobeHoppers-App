...
-- Transactionally replace one private trip, its ordered legs, and Hopper assignments.
-- Uses optimistic concurrency via trips.updated_at.
...
begin
  ...
  if p_expected_updated_at is null then
    raise exception 'The trip revision is missing. Reload the trip before editing.' using errcode = '40001';
  end if;

  if current_updated_at is distinct from p_expected_updated_at then
    raise exception 'This trip changed in another session. Reload before saving.' using errcode = '40001';
  end if;
  ...
  loop
    if nullif(trim(location_item->>'client_id'), '') is null or nullif(trim(location_item->>'name'), '') is null then
    ...
    if resolved_location_id is null then
      ...
      values (
        trim(location_item->>'name'), nullif(trim(location_item->>'region'), ''),
        nullif(trim(location_item->>'country'), ''), nullif(trim(location_item->>'continent'), ''),
        (location_item->>'latitude')::double precision, (location_item->>'longitude')::double precision
  ...
  update public.trips
     set title = trim(p_trip->>'title'),
         ...
         updated_at = now()
   where id = p_trip_id and map_id = p_map_id;
  ...
  loop
    ...
    insert into public.trip_legs (
      ...
      departure_date, arrival_date, route_label, notes, route_geometry, route_provider, route_version
    ) values (
      p_trip_id, from_location_id, to_location_id,
      ...
      nullif(leg_item->>'route_label', ''), nullif(leg_item->>'notes', ''),
      case when leg_item ? 'route_geometry' then leg_item->'route_geometry' else null end,
      nullif(leg_item->>'route_provider', ''), nullif(leg_item->>'route_version', '')
    );
  end loop;

  foreach hopper_id in array coalesce(p_hopper_ids, array[]::uuid[])
  loop
    insert into public.trip_hoppers (trip_id, hopper_id) values (p_trip_id, hopper_id);
  end loop;
...
comment on function public.update_private_trip(uuid, uuid, timestamptz, jsonb, jsonb, jsonb, uuid[]) is
  'Atomically updates one private GlobeHoppers trip with optimistic concurrency, replacing its ordered legs and Hopper assignments.';
